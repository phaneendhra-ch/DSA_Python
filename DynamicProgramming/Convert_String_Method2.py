"""
    ...
    --> Now we are recurring from the last index from both the strings
"""

def convert_string(index1,index2,string1,string2,dict_):

    if (index1 == 0):                               # if we have reached to the start of s1
        return index2                               # then delete the remaining elements from S2


    if (index2 == 0):                               # if we have reached to the start of s2
        return index1                               # then add the remaining elements of S1 to S2


    if (string1[index1-1] == string2[index2-1]):        #if we are at same characters( then increment index value of both string1 and string2 by 1)
        ...
        if dict_key not in dict_:
            ...
            delete_operation = 1 + convert_string(index1-1 ,index2,string1,string2,dict_)         # for delete then move S2 index to next one
            insert_operation = 1 + convert_string(index1 ,index2-1 ,string1,string2,dict_)        # for add then move S1 index to next one
...
if __name__ == '__main__':
    ...
    print(f"Minimum Operations required to convert S2 to S1 : {convert_string(len(string1),len(string2),string1,string2,{})}")
